...
// Known reported scam databases / blacklist handles
const KNOWN_SCAM_INSTAGRAM_HANDLES: Record<
  string,
  {
    riskScore: number;
...
const KNOWN_SCAM_WHATSAPP_NUMBERS: Record<
  string,
  {
...
/**
 * Extract clean handle from user input (supports @handle, instagram.com/handle, URL)
 */
...
/**
 * Comprehensive Instagram Account Authenticity Analyzer
 */
export async function analyzeInstagramProfile(
  ...
  // Dynamic Heuristics for arbitrary handles
  let riskScore = 0;
  const riskSignals: InstagramAnalysisResult['riskSignals'] = [];
  const recommendations: string[] = [];

  // 1. Keyword analysis in handle
  ...
  let matchedKeywords = suspiciousKeywords.filter((kw) => handle.includes(kw));
  ...
  if (knownScam) {
    ...
    riskSignals.push({
      ...
      description: `Unauthorized unauthorized clone pretending to sell discount ${knownScam.impersonatedBrand} items.`,
      severity: 'HIGH',
      points: 20
    });
  } else {
    ...
    // Default heuristics for suspicious social shops
    riskScore += 30; // base suspicion for unverified DM store
    riskSignals.push({
      title: 'Off-Platform WhatsApp Redirection Pattern',
      description: 'Seller conducts financial transactions via WhatsApp DM and personal UPI instead of a verified payment gateway.',
      severity: 'HIGH',
      points: 25
    });
    ...
    riskSignals.push({
      ...
      description: 'Scam pages frequently buy 10,000-50,000 bot followers while turning off comments to hide victim complaints.',
      severity: 'MEDIUM',
      points: 15
    });
  }
  ...
  return {
    ...
    redirectionAnalysis: {
      ...
      warningNote:
        '⚠️ CRITICAL WARNING: This page uses Instagram merely as bait and lures buyers into private WhatsApp chat to request non-refundable UPI transfers.'
    },
    recommendations
  };
}
...
/**
 * Comprehensive WhatsApp Number & UPI Fraud Analyzer
 */
...
/**
 * Get recent social threats (Instagram handles + WhatsApp numbers)
 */
